...
```mermaid
flowchart TD
    A[💰 Wager<br/>10–1,000 credits] --> B[🎰 Spin]
    B --> C[🔎 Evaluate 20 positions]
    C --> D{Winning combination?}

    D -->|No| G[🏁 Settle]
    D -->|Yes| E[💥 Explode winning symbols]

    E --> F[⬇️ Cascade]
    F --> H[🔥 Increase multiplier]
    H --> C

    G --> I{Gamble?}
    I -->|No| J[💳 Bank winnings]
    I -->|Yes| K[🎲 Gamble Ladder]
```
